...
hum = driver.find_element_by_xpath('//*[@id="searchTypeSelectDiv"]/div[3]/div[3]/a/img')
hum.click()
#input search keword
time.sleep(0.5)
    ...
    #next page
        ...
        # infor patent
            for i in range(30):
                ...
                baseUrl = f"http://www.kipris.or.kr/kpat/remoteFile.do?method=downloadImage&applno={e}"
                driver.get(baseUrl)
                time.sleep(0.5)

    #do file download
                # baseUrl = "http://beginner.kipris.or.kr"
                # srcUrl = f"/beginner/remoteFile.do?method=bigFrontDraw&applno={e}"
                # img_url = baseUrl + srcUrl
                # driver.get(img_url)
                # time.sleep(0.5)
